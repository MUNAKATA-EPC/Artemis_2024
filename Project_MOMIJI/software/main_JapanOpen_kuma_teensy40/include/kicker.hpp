...
public:
    ...
    void tick()
    {
        ...
        else
        {
            ...
            else if(Kick_Timer.get_value() <= start_time + 200)
            {
                analogWrite(_charge_pin, 0);
                analogWrite(_kick_pin, 0);
            }
            else if(Kick_Timer.get_value() <= start_time + 250)
            {
                analogWrite(_charge_pin, 0);
                analogWrite(_kick_pin, 1023);
            }
            else if(Kick_Timer.get_value() <= start_time + 270)
            {
                analogWrite(_charge_pin, 0);
                analogWrite(_kick_pin, 0);
            }
            else if(Kick_Timer.get_value() <= start_time + 250 + delay_time)
            {
                analogWrite(_charge_pin, 1023);
                analogWrite(_kick_pin, 0);
            }
