...
public:
    ...
    {
        int line_speed = speed - 20;

        if(Line_Timer.get_value() <= 50 || bIsLineHalfOut)
        {
            line_speed = 100;
        }
        ...
        if(Line_Timer.isticking())
        {
            ...
            else
            {
                ...
                //緊急回避用
                if(Line_Timer.get_value() >= HalfOutTime + 300 && bIsLineHalfOut && !bIsMaybeHalfOut && Line_Deg == -1 && Line_DistanceBefore == 0)
                {
                    bIsLineHalfOut = false;
                }
