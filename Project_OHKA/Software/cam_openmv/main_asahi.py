...
threshold_for_goal_yellow =(66, 99, -61, -18, 17, 95) # ゴールの色取り用変数(黄色)
threshold_for_goal_blue = (35, 54, -7, 8, -52, -25)     # ゴールの色取り用変数(青色)

screen_center = [172, 122]                  # 画面の中央座標
...
sensor.set_auto_whitebal(False, rgb_gain_db=(0, 0, -5))
...
while(True):
    ...
    img.draw_circle(screen_center[0], screen_center[1],200,[0,0,0],150)
    img.draw_circle(screen_center[0], screen_center[1],45,[0, 0, 0], 1, 1)
    ...
    for blob in img.find_blobs([threshold_for_goal_yellow], pixels_threshold=50, area_threshold=50, merge=True,margin=25):
    ...
    for blob in img.find_blobs([threshold_for_goal_blue], pixels_threshold=250, area_threshold=250, merge=True,margin=50):
    ...
    for blob in img.find_blobs([threshold_for_court], pixels_threshold=250, area_threshold=250, merge=True,margin=50):
